...
def _is_dev() -> bool:
    try:
        r = subprocess.run(
            ["git", "show", "-s", "--pretty=%D", "HEAD"], capture_output=True
        )
        if r.returncode != 0:
...
class MaybeMiniGallery(MiniGallery):
    def run(self) -> List[str]:
        ...
        for obj in obj_list:
            path = os.path.join(
                "/", backreferences_dir, f"{obj}.examples"
...
def _get_thumbnails(root: Union[str, Path]) -> Dict[str, str]:
    ...
    for fname in root.glob("**/*.py"):
        ...
        thumb_fname = f"{name}.png"
        if (thumb_path / path / "images" / "thumb" / thumb_fname).is_file():
            # res[str(fname)[:-3]] = f"_images/{thumb_fname}"
            res[str(fname)[:-3]] = path + "/images/thumb/" + thumb_fname
    res["**"] = "_static/img/BaSiC.png"
    return res
...
def _fetch_notebooks(repo_url: str) -> None:
    def copy_files(repo_path: Union[str, Path]) -> None:
        ...
        for dirname in ["auto_examples", "auto_tutorials", "gen_modules"]:
    ...
    notebooks_local_path = Path(
        os.environ.get(
            ...
            HERE.absolute().parent.parent.parent / "squidpy_notebooks",
        )
    )
    # try:
    ...
    if not download:
        ...
        info(
            f"Not fetching notebooks from remove because `SQUIDPY_DOWNLOAD_NOTEBOOKS={download}`"
        )
        return
